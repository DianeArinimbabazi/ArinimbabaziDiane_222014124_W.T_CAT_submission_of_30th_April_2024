...
<body style="background-image: url('./Images/image16.png');background-repeat: no-repeat;background-size:cover;">
  <header>
    <h1>SUPPLIERS</h1>
  </header>
  ...
  <section>
    ...
    <form method="post" onsubmit="return confirm('Are you sure you want to insert this record?');">
      ...
      <input type="number" id="supplied" name="supplied"><br><br>
    ...
    <?php
      ...
      if ($_SERVER["REQUEST_METHOD"] == "POST") {
          ...
          $stmt = $connection->prepare("INSERT INTO supprier (id, names, contact, supplied) VALUES (?, ?, ?, ?)");
          $stmt->bind_param("isss", $id, $names,  $contact, $supplied);
          ...
          $supplied= htmlspecialchars($_POST['supplied']); // Prevent XSS
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<tr>
                  ...
                  <td><a style='padding:4px' href='Delete_supprier.php?id=$id'>Delete</a></td> 
                  <td><a style='padding:4px' href='update_supprier.php?id=$id'>Update</a></td> 
                </tr>";
            }
        } else {
